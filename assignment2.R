...
library(lubridate)
library(readxl)
# Updated the code below to Modify the m11 model to incorporate the covid19 period as holiday
setwd("/Users/vishwasdhanda/Desktop/heltcare_project/forecasting/Assignment 2/Data and Script 3/")
...
tsa_test <- tsa_test %>% rename(ds = ds, y = travel_test)
forecast_test_m11 <- forecast11 %>% filter(ds >= min(tsa_test$ds) & ds <= max(tsa_test$ds))
rmse_m11 <- sqrt(mean((forecast_test_m11$yhat - tsa_test$y)^2))
print(paste("RMSE of the original model (m11):", rmse_m11))

#m11_modified

# Load the train and test datasets
tsa_train <- read_excel("W09e_tsa.xlsx", sheet = "train")
tsa_test <- read_excel("W09e_tsa.xlsx", sheet = "test")

# Format the training data for Prophet
tsa_train <- tsa_train %>% rename(ds = ds, y = travel_train)

# Define the COVID period as a holiday
holidays <- data.frame(
  holiday = 'covid',
  ds = seq(as.Date('2020-03-15'), as.Date('2022-03-01'), by = 'days'),
  lower_window = 0,
  upper_window = 0
)

# Initialize and fit the modified Prophet model with holidays
m11_modified <- prophet(holidays = holidays)
m11_modified <- fit.prophet(m11_modified, tsa_train)

# Make predictions on the test set
future <- make_future_dataframe(m11_modified, periods = 81)
forecast_modified <- predict(m11_modified, future)

# Calculate RMSE for the modified model
tsa_test <- tsa_test %>% rename(ds = ds, y = travel_test)
forecast_test <- forecast_modified %>% filter(ds >= min(tsa_test$ds) & ds <= max(tsa_test$ds))
rmse_modified <- sqrt(mean((forecast_test$yhat - tsa_test$y)^2))
print(paste("RMSE of the modified model:", rmse_modified))

# Plot the modified forecast
plot(m11_modified, forecast_modified)
prophet_plot_components(m11_modified, forecast_modified)
# Long-term forecast
m12 = prophet(tsa)
